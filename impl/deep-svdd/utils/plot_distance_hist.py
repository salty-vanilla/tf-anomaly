...
def plot(dst_path, csv_path, bins=30, norm_hist=True):
    ...
    df = df[['label', 'distance']]

    inlier_normality = df[df.label == 'inlier']['distance']
    outlier_normality = df[df.label == 'outlier']['distance']
    ...
    sns.distplot(inlier_normality,
                 ...
                 color=blue,
                 norm_hist=norm_hist,
                 bins=np.linspace(0, float(df['distance'].max()), bins),
                 hist_kws={'rwidth': 1.0,
                           'alpha': 0.8,
                           'stacked': False})
